...
public class TntTracker implements Listener {
    ...
    public static void setOwner(Entity entity, Player player) {
        entity.setMetadata("source", new FixedMetadataValue(Warzone.getInstance(), player.getUniqueId()));
    }
    ...
    public void onExplosionPrime(ExplosionPrimeEvent event) {
        if (event.getEntity().getType() == EntityType.PRIMED_TNT) {
            ...
            if (tntPlaced.containsKey(location.getBlockX() + "," + location.getBlockY() + "," + location.getBlockZ())) {
                ...
                event.getEntity().setMetadata("source", new FixedMetadataValue(Warzone.getInstance(), playerUUID));
...
//                            F.debug("found placer: " + Bukkit.getServer().getPlayer(playerUUID));
                            entity.setMetadata("source", new FixedMetadataValue(Warzone.getInstance(), playerUUID));
                        }
                    }
                }
            }

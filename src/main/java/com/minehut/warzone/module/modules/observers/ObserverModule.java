...
//                        ChatColor.GREEN + "" + ChatColor.BOLD + new LocalizedChatMessage(ChatConstant.UI_TEAM_SELECTION).getMessage(event.getPlayer().spigot().getLocale()),
//                        Collections.singletonList(ChatColor.DARK_PURPLE + new LocalizedChatMessage(ChatConstant.UI_TEAM_JOIN_TIP).getMessage(event.getPlayer().spigot().getLocale())));
//                event.getPlayer().getInventory().setItem(1, picker);
//            }
        }
    }

    @EventHandler
    public void onClickCreativeInventory(InventoryClickEvent event) {
    ...
    public void openInventory(Player viewer, Player view, boolean message){
        if (testObserver(viewer) && !testObserver(view)) {
            ...
            viewer.sendMessage(ChatColor.RED + new LocalizedChatMessage(ChatConstant.ERROR_INVENTORY_NOT_VIEWABLE).getMessage(viewer.spigot().getLocale()));
        }
    }
    ...
    //todo: cancel experience pickup for observers
//    @EventHandler

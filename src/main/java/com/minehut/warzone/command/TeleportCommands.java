    ...
    public static void teleport(final CommandContext cmd, CommandSender sender) throws CommandException {
        if (sender instanceof Player) {
            ...
            if (cmd.argsLength() == 1) {
                if (sender.hasPermission("cardinal.teleport") || !GameHandler.getGameHandler().getMatch().getState().equals(MatchState.PLAYING) || (Teams.getTeamByPlayer(player).isPresent() && Teams.getTeamByPlayer(player).get().isObserver())) {
                    try {
                        ...
                        sender.sendMessage(ChatColor.YELLOW + new LocalizedChatMessage(ChatConstant.GENERIC_TELEPORTED).getMessage(player.spigot().getLocale()));
                    } catch (NullPointerException e) {
                        throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_NO_PLAYER_MATCH).getMessage(player.spigot().getLocale()));
                    }
                } else {
                ...
                if (sender.hasPermission("cardinal.teleport")) {
                    try {
                        ...
                        sender.sendMessage(ChatColor.YELLOW + new LocalizedChatMessage(ChatConstant.GENERIC_TELEPORTED).getMessage(player.spigot().getLocale()));
                        if (player != from) {
                            from.sendMessage(ChatColor.YELLOW + new LocalizedChatMessage(ChatConstant.GENERIC_TELEPORTED_BY, player.getDisplayName()).getMessage((from.spigot().getLocale())));
                        }
                    } catch (NullPointerException e) {
                        throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_NO_PLAYER_MATCH).getMessage(player.spigot().getLocale()));
                    }
                ...
                if (sender.hasPermission("cardinal.teleport")) {
                    ...
                    if ( -30000000 < x && x < 30000000) {
                        if ( -30000000 < z && z < 30000000) {
                            ...
                            player.sendMessage(numTooBig.toString());
                        }
                    } else {
                        TranslatableComponent numTooBig = new TranslatableComponent("commands.generic.num.tooBig");
                        numTooBig.addWith("" + x);
                    ...
                    try {
                        ...
                        if ( -30000000 < x && x < 30000000) {
                            if ( -30000000 < z && z < 30000000) {
                                ...
                                sender.sendMessage(new LocalizedChatMessage(ChatConstant.GENERIC_TELEPORTED).getMessage(player.spigot().getLocale()) + ChatColor.YELLOW);
                                if (player != teleporting) {
                                    teleporting.sendMessage(ChatColor.YELLOW + new LocalizedChatMessage(ChatConstant.GENERIC_TELEPORTED_BY, player.getDisplayName()).getMessage((teleporting.spigot().getLocale())));
                                }
                            } else {
                                TranslatableComponent numTooBig = new TranslatableComponent("commands.generic.num.tooBig");
                                numTooBig.addWith("" + z);
            ...
            try {
                ...
                throw new CommandException(new LocalizedChatMessage(ChatConstant.ERROR_NO_PLAYER_MATCH).getMessage(((Player) sender).spigot().getLocale()));
            }
        } else if (cmd.argsLength() == 3) {
            if (!sender.hasPermission("cardinal.teleport")) {
                throw new CommandPermissionsException();
